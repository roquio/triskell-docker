...
PORTAL_HOSTS=${PORTAL_HOSTS:-portal}
...
if [ ! -f $HTTPD_HOME/configured ]; then
	...
	if [ "$HTTPS_PROVIDED" = false ]; then
		...
		if [ ! -f $CERTS_FILES/server.crt ]; then
	    	openssl req -nodes -newkey rsa:2048 -keyout $CERTS_FILES/server.key -out $CERTS_FILES/server.csr -subj "/C=FR/ST=Loire-Atlantique/L=Nantes/O=OSIVIA/OU=Portal/CN=$PUBLIC_HOST"
